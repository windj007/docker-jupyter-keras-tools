...
RUN echo 'export PATH=/root/.pyenv/bin:$PATH' | tee -a /root/.bash_profile >> /root/.bashrc
RUN echo 'eval "$(pyenv init -)"' | tee -a /root/.bash_profile >> /root/.bashrc
...
# RUN apt-get install -yqq libboost-python-dev libboost-program-options-dev
# RUN /bin/bash -l -c 'pip install -U vowpalwabbit'
#
